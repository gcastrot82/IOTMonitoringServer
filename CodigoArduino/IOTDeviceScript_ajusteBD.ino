...
const char ssid[] = "GerardoCastro"; // TODO cambiar por el nombre de la red WiFi
// Contraseña de la red WiFi
const char pass[] = "G3r@rdo2021"; // TODO cambiar por la contraseña de la red WiFi
...
const char MQTT_HOST[] = "3.88.210.15"; // TODO Reemplace ip.maquina.mqtt por la IP del bróker MQTT que usted desplegó. Ej: 192.168.0.1
const int MQTT_PORT = 8082;
const char MQTT_USER[] = USER;
//Contraseña de MQTT
const char MQTT_PASS[] = "Gerardo123"; // TODO Reemplace ContrasenaMQTT por la contraseña correpondiente al usuario especificado.

//Tópico al que se recibirán los datos
// El tópico de publicación debe tener estructura: <país>/<estado>/<ciudad>/<usuario>/out
const char MQTT_TOPIC_PUB[] = "colombia/valle/cali/" USER "/out"; //TODO Reemplace el valor por el tópico de publicación que le corresponde.
// El tópico de suscripción debe tener estructura: <país>/<estado>/<ciudad>/<usuario>/in
const char MQTT_TOPIC_SUB[] = "colombia/valle/cali/" USER "/in"; //TODO Reemplace el valor por el tópico de suscripción que le corresponde.
...
float readLuminosidad() {
  // Se lee la luminosidad relativa
...
bool checkMeasures(float t, float h, float l) {
  // Se comprueba si ha habido algún error en la lectura
    if (isnan(t) || isnan(h) || isnan(l)) {
      Serial.println("Error obteniendo los datos del sensor DHT11 y LDR");
      return false;
    }
    return true;
}
...
void measure() {
  if ((millis() - measureTime) >= MEASURE_INTERVAL * 1000 ) {
    ...
    Serial.println(" El mensaje es:  "+message);
